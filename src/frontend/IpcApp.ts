import { AsyncMethodsOf, PickAsyncMethods, PromiseReturnType } from './UtilityTypes'
import {
  IpcAppChannel, IpcAppNotifications,
  IpcInvokeReturn,
  IpcListener,
  IpcSocketFrontend,
  ipcChannel,
...
export class IpcApp {
  ...
  public static async callIpcHost<T extends AsyncMethodsOf<IpcAppFunctions>>(
    ...
    return await this.callIpcChannel(
...
export abstract class NotificationHandler {
  ...
  public registerImpl(): RemoveFunction {
    return IpcApp.addListener(this.channelName, (_evt: Electron.Event, funcName: string, ...args: any[]) => {
      const func = (this as any)[funcName]
      if (typeof func !== 'function') {
        throw new Error(`Method "${ this.constructor.name }.${ funcName }" not found on NotificationHandler registered for channel: ${ this.channelName }`)
      }
